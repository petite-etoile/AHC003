...
int get_range(int query){
    const int MAX_QUERY = 1000;
    const int start_range = 10;
    const int end_range = 5;
    
    int range = (start_range*(MAX_QUERY-query) + end_range*(query-0) ) / MAX_QUERY;
    // debug(range)
    return range;
}
...
bool flag = false;
vector<vector<pair<int,int64>>> get_edge(vector<vector<pair<int,int64>>> const& estimated_edge, int query){
    const int64 init_cost = 3000; //考慮するべき
    vector<vector<pair<int,int64>>> edge(node_num);
    
    int const min_count = 100;
    // int cost_range = get_range(query); //前後何個の情報で辺の重みを決定するか
    ...
    REP(h,N){
        ...
        REP(w,N-1){
            pair<int,int> next_ = make_pair(h,w+1);
            pair<int,int64> info_nears; // //前後K個での和

            int cost_range = 0;
            do{
                info_nears = sub_pairs(cumsum[min(N-1, w+cost_range)], cumsum[max(0, w-cost_range)]);
            }while(cost_range++<N and info_nears.first <= min_count);
...
int main(){
    ...
    REP(query,Query_num){
        ...
        auto edge = get_edge(estimated_edge, query);
        // show_edge(edge);
