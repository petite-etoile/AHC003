...
vector<vector<pair<int,int64>>> get_edge(vector<vector<pair<int,int64>>> const& estimated_edge, int query){
    ...
    const int cost_range = get_range(query); //前後何個の情報で辺の重みを決定するか
    const int coef = 0; //自分の辺の情報の重み付け
...
void re_eval_costs(int source, int target, string ans, int given_path_length, vector<vector<pair<int,int64>>> & estimated_edge){
    ...
    for(char move:ans){
        ...
        int coef = 1000*RunLength[idx]/ans.size(); //行動の中でその行/列の辺をどれだけ使っているか
        // debug(RunLength)
        // debug(coef)
        // debug(idx)
...
/*
    ...
    cost_range:8
    coef:5
    average 926087251.446

    init_cost:3000
    cost_range:8
    coef:0
    average 925776130.795


    init_cost:3000
    cost_range:8
    coef:10
    average 925666695.796
