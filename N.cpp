...
string get_L_path(int const& source, int const& target){
    string path = "";
    string LRUD = "LRUD";

    random_device seed_gen;
    mt19937 engine(seed_gen());
    shuffle(LRUD.begin(), LRUD.end(), engine);
    for(auto dir: LRUD){
        int s_h, s_w, t_h, t_w;
        tie(s_h, s_w) = decode(source); tie(t_h, t_w) = decode(target);

        if(dir == 'L'){
            if(t_w < s_w ) path += string(s_w-t_w, dir);
        }else if(dir == 'R'){
            if(t_w > s_w ) path += string(t_w-s_w, dir);
        }else if(dir == 'U'){
            if(t_h < s_h ) path += string(s_h-t_h, dir);
        }else{
            if(t_h > s_h ) path += string(t_h-s_h, dir);
        } 
    }
...
string solve(int const& source, int const& target, vector<vector<pair<int,int64>>> const& edge, int query){
    string ans;
    const int first_servey = 100;
    if(query<first_servey){
        ans = get_L_path(source, target);
    }else{
...
int main(){
    ...
    REP(query,Query_num){
        ...
        // debug(source)
        // debug(target)
        string ans = solve(encode(source), encode(target), edge, query);
        cout << ans << endl;
...
/*
    ...
    average 931393332.2
    first_servey:50
    average 932315065.22
    first_servey:100
    average 935452432.08
    first_servey:150
    average 932464462.54
